...
def generate_benchmark_code(M_list, tile_sizes, stride, local_size):
    ...
    for M in M_list:
        ...
        code.append(f"")
        code.append(f"    // Allocate host memory")
        code.append(f"    auto* A_host = sycl::malloc_host<T>(K * M, q);")
        code.append(f"    auto* B_host = sycl::malloc_host<T>(K * M, q);")
        code.append(f"    auto* C_host = sycl::malloc_host<T>(M * M, q);")
        code.append(f"")
        code.append(f"    // Allocate device memory")
        code.append(f"    auto* A = sycl::malloc_device<T>(K * M, q);")
        code.append(f"    auto* B = sycl::malloc_device<T>(K * M, q);")
        code.append(f"    auto* C = sycl::malloc_device<T>(M * M, q);")
        code.append(f"")
        code.append(f"    // Initialize data on host")
        code.append(f"    for (int i = 0; i < K * M; ++i) {{")
        code.append(f"      A_host[i] = T(rand()) / RAND_MAX - 0.5;")
        code.append(f"      B_host[i] = T(rand()) / RAND_MAX - 0.5;")
        code.append(f"    }}")
        code.append(f"")
        code.append(f"    // Copy to device")
        code.append(f"    q.memcpy(A, A_host, K * M * sizeof(T)).wait();")
        code.append(f"    q.memcpy(B, B_host, K * M * sizeof(T)).wait();")
        code.append(f"")
        code.append(f"    auto C_ref = compute_reference(K, M, A_host, B_host);")
        code.append(f"")
        ...
        code.append(f"      q.memcpy(C_host, C, M * M * sizeof(T)).wait();")
        code.append(f"      bool correct = check_correctness(C_ref, C_host, M);")
        code.append(f"")
        ...
        code.append(f"    sycl::free(A_host, q);")
        code.append(f"    sycl::free(B_host, q);")
        code.append(f"    sycl::free(C_host, q);")
        code.append(f"  }}")
        code.append(f"")
